...
The proof of concept also hooks `LoadingMenu::AdvanceMovie` at vtable slot `0x05` for presentation only. The original engine update runs first; the post-update pass duplicates `_root.Menu_mc.LevelMeterRect.LevelProgressBar` as `LevelMeterRect.SkyrimLoadProgress`, preserving the active loading-menu skin, and positions it six pixels below the source meter. It reproduces `Components.Meter.SetPercent` by interpolating between the duplicate's `Empty` and `Full` frame labels, including skins whose frame range runs in descending order. Queue state is never polled from this hook.
