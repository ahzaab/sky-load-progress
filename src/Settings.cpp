...
void Settings::Load()
{
    ...
    try {
        ...
        logger::info("loading meter layout: x={:.1f}% y={:.1f}% width={:.1f}%",
            progressBar.xPercent, progressBar.yPercent, progressBar.widthPercent);
...
const Settings::ColdTransition& Settings::GetColdTransition(std::string_view a_editorID) const
{
    for (const auto& rule : cellRules) {
        if (MatchesPattern(a_editorID, rule.pattern)) {
            logger::info("cell transition rule matched: editorID='{}' pattern='{}'", a_editorID, rule.pattern);
